...
* The routing tick calls `_session_sample_tick()` every tick while
  a session is recording — that's what feeds the OGB-param
  stream. The motor stream is pushed automatically
...
class SessionsFacade:
    ...
    def _session_sample_tick(self) -> None:
        ...
        recording. Pulls the current OGB-param snapshot and pushes it
        through the logger.

        Motor events are pushed by the broadcast callback in
        `_session_on_motor`; this method handles the stream that
        isn't naturally per-motor."""
        if not self.is_session_logging_active():
            return
