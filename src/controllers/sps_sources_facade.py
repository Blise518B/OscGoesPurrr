...
class SpsSourcesFacade:
    ...
    def _get_sps_source_map(self) -> Dict[str, Dict[str, Any]]:
        ...
        motor router each tick so a selected synthetic source
    ...
    def get_sps_source_names_by_type(self) -> Dict[str, List[str]]:
        ...
        mirroring get_detected_zones() so every zone picker
    ...
    def get_live_zone_strengths(
        ...
        Same math the motor router runs per
    ...
    def get_live_parameter_values(self, names: Sequence[Any]) -> List[float]:
        """Read a batch of RAW parameters as 0..1 values against ONE snapshot.

        The zone-strength sibling above answers "how strong is this OGB
        zone"; this answers "what is this contact receiver reading". Used by
        a source that is a plain contact parameter to paint
        the same live activity rings. `normalize_osc_value` matches what
        those routers do per tick, so the ring shows the routed value rather
        than an approximation. Missing / malformed params read 0.0."""
        if not names:
            return []
        params = store.get_all_parameters() or {}
        out: List[float] = []
        for name in names:
            ...
            out.append(normalize_osc_value(raw) if raw is not None else 0.0)
        return out
