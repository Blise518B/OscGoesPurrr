...
class OscFacade:
    ...
    def _osc_flow_state(self):
        """Reception snapshot for the 518 fallback ladder, or None when no
        OSC manager exists yet. Called on the fallback's own thread every
        couple of seconds — plain attribute reads, no locking, no I/O."""
        manager = getattr(self, "osc_manager", None)
        if manager is None:
            return None
        try:
            return manager.get_flow_state()
        except Exception:
            return None

    def get_osc_link_state(self) -> str:
        """Facade: truthful link classification for the status pill
        (osc_link_state module). Rides the periodic UI tick — attribute
        reads only, no locking, no I/O."""
        manager = getattr(self, "osc_manager", None)
        if manager is None:
            return "disconnected"
        try:
            return manager.get_link_state()
        except Exception:
            return "disconnected"
    ...
    def get_osc_diagnostics(self) -> Dict[str, Any]:
        ...
        if not getattr(self, "osc_manager", None):
            return {}
        try:
            diag = self.osc_manager.get_diagnostics()
        except Exception as e:
            return {"error": f"{type(e).__name__}: {e}"}
        # Fold in which 518 tier we're on, so "silent but still receiving"
        # reads as the fallback working rather than as a mystery.
        fallback = getattr(self, "router518", None)
        if fallback is not None:
            try:
                diag["router518"] = fallback.get_state()
            except Exception:
                pass
        return diag
    ...
    def on_osc_message(self, address: str, value):
        ...
        elif address == OGP_STRENGTH_PARAMETER:
            self.thread_queue.put(("ogp_strength", value))
        elif address == OGP_OFF_PARAMETER:
            self.thread_queue.put(("ogp_off", bool(value)))
        elif address == OGP_SLEEP_PARAMETER:
            self.thread_queue.put(("ogp_sleep", bool(value)))
