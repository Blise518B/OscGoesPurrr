...
class IntifaceFacade:
    ...
    def update_device_target(self, device_name: str, value: float, motor_index: int):
        ...
        if (device_name in self._muted_devices
            ...
            engine_value = 0.0
        elif self.get_master_scale() <= 0.0:
            # Off — the panic switch. The master scale is applied upstream
            # now (motor_router folds it into each chain's Output stage, so
            # a chain's minimum-output floor can sit downstream of it), but
            # silence must never wait for a recompute: zero it here too.
            engine_value = 0.0
            if self.is_ogp_test_active():
                # The OGP/Test floor still rides on top so the menu's
                # connectivity pulse works even in Off mode.
                engine_value = max(engine_value, self._test_floor(
                    device_name, motor_index))
        else:
            # Already carries the master scale (see above) and the chain's
            # Output band. The OGP/Test floor rides on top.
        ...
        # meter while the device is muted. It now reads post-strength and
        # post-Output-band — that value is the toy's, which is what a meter
        # sitting on the motor card should be showing.
        self.ui.update_motor_vibe(device_name, motor_index, real_value)

    def _test_floor(self, device_name: str, motor_index: int) -> float:
        """The OGP/Test pulse level for THIS motor, remapped into its
        calibrated output band.

        A fixed raw level stops meaning the same thing across a rig once
        motors are calibrated: on a motor whose band starts above the pulse
        level, a raw floor lands in the dead zone and the connectivity
        check reads as a dead toy. Never raises into dispatch — an
        unroutable motor falls back to the raw level, which is what the
        pulse did before bands existed."""
        level = self.get_ogp_test_level()
        try:
            return self.motor_router.map_into_output_band(
                device_name, motor_index, level)
        except Exception:
            return level
    ...
    def trigger_purr_check(self):
        ...
        on the engine loop, so waiting for it here -- as this used to, for
        up to 3 s -- froze the whole window on every click. A failure is
        logged from the engine thread; log_message marshals to the GUI."""
        if not (self.async_loop and self.haptic_engine
                and self.haptic_engine.is_connected):
            self.log_message("Test all: no toys connected.")
            return
        try:
            future = asyncio.run_coroutine_threadsafe(
                self.haptic_engine.async_purr_check(), self.async_loop)
        except Exception as e:
            self.log_message(f"Purr-Check failed: {e}")
            return

        def _report(fut):
            if fut.cancelled():   # loop torn down mid-check; nothing to say
                return
            exc = fut.exception()
            if exc is not None:
                self.log_message(f"Purr-Check failed: {exc}")
