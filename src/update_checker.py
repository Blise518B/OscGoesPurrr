...
def check_for_update(current_version: str,
    ...
    Returns {"available": bool, "latest": str, "url": str, "asset": dict
    or None} — `latest` is the release tag without its leading "v", `url`
    the release page, `asset` the downloadable .exe when the release has
    one attached (see updater.pick_exe_asset) — or None on ANY failure (no
    network, rate-limited, junk tag, junk current version, requests
    missing). Blocking (one HTTPS round-trip, bounded by `timeout_s`);
    ...
    try:
        ...
        # The downloadable exe, when the release has one. Imported lazily
        # and guarded so a problem in the updater can never turn a working
        # notice-only check into no check at all.
        asset = None
        try:
            from updater import pick_exe_asset
            asset = pick_exe_asset(payload.get("assets"))
        except Exception:
            asset = None
        return {
