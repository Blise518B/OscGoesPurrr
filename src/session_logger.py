...
to per-session JSONL files for later analysis.
...
* The hot path (router tick) calls `log_motor`, `log_ogb`,
  from the routing thread. These just append to an
...
class SessionLogger:
    ...
    def start(self, metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        with self._lock:
            ...
            header = {
                "type": "header",
                # 2: motor rows gained a `contact` column. `out` became the
                # toy's drive value when the global strength moved into the
                # chain's Output stage, so schema-1 `out` and schema-2 `out`
                # are not the same quantity — the version is how a reader
                # tells them apart.
                "schema": 2,
    ...
    def log_motor(self, t_ms: float, device: str, motor: int,
                  ...
                  mixed: float, out: float,
                  contact: float = 0.0) -> None:
        """One motor's per-tick chain snapshot.

        `out` is the value the toy was driven with — it carries the global
        strength and the motor's output band, so it is NOT comparable
        between two recordings made at different strength settings.
        `contact` is the same tick one stage earlier (pre-gain,
        pre-strength, pre-band): the strength- and calibration-independent
        signal, in the same frame of reference as d_raw..mixed. Optional
        so the six-field call still works."""
        if not self._running:
            return
        event = {
            "type":     "motor",
            ...
            "contact":  round(float(contact),  FLOAT_DECIMALS),
        }
        self._enqueue(event)
