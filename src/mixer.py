...
def sample_pattern(freq_hz: float, amp: float, waveform: str,
    ...
    Waveforms:
      ...
      * `random`   — strokes at `freq_hz` whose depth changes every
        cycle, riding a slow wander between quiet and intense sections
        (`_random_pattern`). Deterministic in `t_s`, so it is still a
        pure function and replays identically.
    ...
    if waveform == "random":
        return float(amp) * _random_pattern(float(freq_hz), float(t_s))
    return 0.0


# `random`: not a fixed shape. A slow wander decides how intense the
# current SECTION is (a new random level every few seconds, eased between
# levels, never fully dead), and every stroke inside it draws its own
# depth -- so the drive visits shallow, deep, quiet and busy passages the
# way a real session does. Hash-based rather than a RNG so the value at a
# time is the same on every call: the sampler stays a pure function.
_RANDOM_SECTION_S = 5.0


def _hash01(n: int) -> float:
    x = math.sin(float(n) * 12.9898 + 78.233) * 43758.5453
...
def _random_pattern(freq_hz: float, t_s: float) -> float:
    """Deterministic 'random' drive in [0, 1] at time `t_s`."""
    sec = t_s / _RANDOM_SECTION_S
    si = math.floor(sec)
    ease = _smoothstep(sec - si)
    a, b = _hash01(int(si)), _hash01(int(si) + 1)
    level = 0.2 + 0.8 * (a + (b - a) * ease)       # the section's intensity
    cyc = freq_hz * t_s
    ci = math.floor(cyc)
    phase = cyc - ci
    depth = 0.4 + 0.6 * _hash01(1000 + int(ci))      # this stroke's depth
    stroke = 0.5 - 0.5 * math.cos(2.0 * math.pi * phase)
    return max(0.0, min(1.0, level * depth * stroke))
