...
def driver_listening(timeout: float = 0.3) -> bool:
    """True when SteamVR is running with the toy driver loaded right now
    (the driver only listens while SteamVR has it). A bare connect-and-close
    on loopback — refused is instant. Ask while the bridge is stopped: the
    driver serves one client at a time."""
    try:
        with socket.create_connection((DRIVER_HOST, DRIVER_PORT), timeout=timeout):
            return True
    except OSError:
        return False
