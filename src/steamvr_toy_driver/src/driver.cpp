...
// ---- How a toy presents itself to SteamVR --------------------------------
//
// A toy must show up in SteamVR's device list WITHOUT ever looking like a
// body tracker: VRChat (and every full-body app) picks up GenericTracker
// devices, so a toy registered as one would hijack a real tracker slot.
// These come from the driver's own default.vrsettings section so the
// presentation can be tuned without a rebuild.

struct ToyPresentation {
    ETrackedDeviceClass deviceClass = TrackedDeviceClass_TrackingReference;
    bool poseValid = false;
    std::string renderModel;
};

static ToyPresentation g_presentation;

static void LoadPresentation() {
    EVRSettingsError err = VRSettingsError_None;
    int32_t cls = VRSettings()->GetInt32("driver_oscgoespurrr", "toy_device_class", &err);
    if (err == VRSettingsError_None && cls > 0)
        g_presentation.deviceClass = (ETrackedDeviceClass)cls;
    err = VRSettingsError_None;
    bool valid = VRSettings()->GetBool("driver_oscgoespurrr", "toy_pose_valid", &err);
    if (err == VRSettingsError_None) g_presentation.poseValid = valid;
    char model[256] = {0};
    err = VRSettingsError_None;
    VRSettings()->GetString("driver_oscgoespurrr", "toy_render_model", model, sizeof(model), &err);
    if (err == VRSettingsError_None) g_presentation.renderModel = model;
}
...
public:
    ...
    EVRInitError Activate(uint32_t unObjectId) override {
        ...
        // Not a tracker: no tracker render model or role, so no app can bind
        // a toy to a body slot. SteamVR still wants every device to have a
        // controller type and input profile; ours declares no inputs at
        // all, so nothing can be bound to a toy either. The render model is
        // optional (empty = nothing drawn in the headset).
        VRProperties()->SetStringProperty(m_propertyContainer, Prop_ControllerType_String,
            "oscgoespurrr_toy");
        VRProperties()->SetStringProperty(m_propertyContainer, Prop_InputProfilePath_String,
            "{oscgoespurrr}/input/oscgoespurrr_toy_profile.json");
        if (!g_presentation.renderModel.empty()) {
            VRProperties()->SetStringProperty(m_propertyContainer, Prop_RenderModelName_String,
                g_presentation.renderModel.c_str());
        }
        ...
        // even though the icon file loads and caches correctly. RunFrame
        // still sends a pose every frame (not valid, unless the
        // presentation settings say otherwise) so the device reads as alive.
    ...
    DriverPose_t GetPose() override {
        return MakeIdentityPose(m_connected.load());
    }

    static DriverPose_t MakeIdentityPose(bool connected) {
        DriverPose_t pose{};
        pose.poseIsValid = g_presentation.poseValid;
        // Without a pose, SteamVR's status window says "Searching..." when a
        // toy is hovered — its wording for any base station with no pose.
        // Reporting Running_OK instead does not change that (tried
        // 2026-09-24), so the result stays honest.
        pose.result = g_presentation.poseValid ? TrackingResult_Running_OK
                                               : TrackingResult_Running_OutOfRange;
        // SteamVR has no RemoveDevice: a toy that leaves the app's list is
        // reported disconnected instead, which takes it off the strip.
    ...
    void SetConnected(bool connected) { m_connected.store(connected); }
...
public:
    EVRInitError Init(IVRDriverContext* pDriverContext) override {
        ...
        LoadPresentation();
        DriverLog("[ogp] ToyProvider::Init (class=%d pose_valid=%d model='%s')",
                  (int)g_presentation.deviceClass, (int)g_presentation.poseValid,
                  g_presentation.renderModel.c_str());
    ...
    void RunFrame() override {
        ...
        for (auto& kv : m_devices) {
            ...
            DriverPose_t pose = ToyDevice::MakeIdentityPose(dev->Connected());
...
private:
    ...
    void IpcThreadMain() {
        ...
        while (!m_stop.load()) {
            // One listening socket for the driver's whole life. Binding a
            // fresh one per client left the old one open as well (Windows
            // lets SO_REUSEADDR sockets share a port), and a reconnecting
            // app landed in the stale socket's backlog: "connected", but
            // never read.
            if (m_listenSock == INVALID_SOCKET && !StartListener()) {
    ...
    void ReplaceDevices(const std::vector<std::shared_ptr<ToyDevice>>& incoming) {
        ...
        // RemoveDevice anyway).
        std::vector<std::shared_ptr<ToyDevice>> added;
        {
            std::lock_guard<std::mutex> lock(m_devicesMu);
            // Toys no longer in the list go disconnected; they come back
            // (same SteamVR slot) if the same serial is sent again.
            for (auto& kv : m_devices) {
                if (incomingMap.find(kv.first) == incomingMap.end())
                    kv.second->SetConnected(false);
            }
            for (auto& kv : incomingMap) {
                auto existing = m_devices.find(kv.first);
                if (existing != m_devices.end()) {
                    existing->second->SetConnected(true);
                    existing->second->UpdateBattery(kv.second->Battery());
                    existing->second->UpdateIcon(kv.second->IconPath());
                    continue;
                }
                m_devices[kv.first] = kv.second;
                added.push_back(kv.second);
            }
        }
        // Announce new devices with the lock RELEASED: SteamVR may activate
        // them on its main thread, and RunFrame (also the main thread) takes
        // m_devicesMu — holding it here could deadlock vrserver into its
        // watchdog abort.
        for (auto& dev : added) {
            VRServerDriverHost()->TrackedDeviceAdded(
                dev->Serial().c_str(),
                g_presentation.deviceClass,
                dev.get());
            DriverLog("[ogp] Added toy '%s' (icon_key='%s')",
                      dev->Serial().c_str(), dev->IconKey().c_str());
        }
    }
};
