...
from osc_link_state import link_state
from osc_router_518 import OscFlowState, ROUTER_PORT as _ROUTER_518_PORT

#: Datagrams re-broadcast by OSC Router 518 reach us from the router's own
#: socket on loopback. Recognising them is what lets the 518 fallback tell
#: "VRChat can still see me" apart from "I'm only alive because the router
#: is feeding me" — see osc_router_518.Router518Fallback.
...
class VRChatOSCManager:
    ...
    # While the DIRECT stream stays dead, repeat the reprobe/re-register
    # self-heals on this cadence instead of giving up after one shot. Direct
    # is the lowest-latency path (the 518 router adds a hop), so we keep
    # trying to win it back for the whole session; the fallback ladder holds
    # its router subscription through each re-publish, so a retry can no
    # longer cut the stream, and it switches back to Tier 0 by itself the
    # moment VRChat's pushes actually resume.
    _SILENT_RETRY_INTERVAL_S = 300.0
    ...
    def __init__(self, local_listen_port: int = 0, bind_all_interfaces: bool = True, rate_limit_hz: float = 20.0):
        ...
        # needs_reply_address: the wrapper classifies each datagram by source
        # (straight from VRChat vs re-broadcast by OSC Router 518) before
        # delegating to the real handler — see _dispatch_incoming.
        self.dispatcher.set_default_handler(self._dispatch_incoming,
        ...
        # Same clock, but counting ONLY packets that came straight from
        # VRChat. Router-518 re-broadcasts land on this same UDP port, and
        # if they counted as healthy direct discovery the 518 fallback would
        # unsubscribe itself the moment it started working. `None` = no
        # direct packet yet this session.
        self._last_direct_packet_time: Optional[float] = None
        self._packets_direct: int = 0
        self._packets_via_router: int = 0
        # Snapshot of _packets_direct at the last watchdog tick, so it can
        # tell "no direct traffic this interval" from "none at all".
        self._direct_at_last_log: int = 0
        # When we (re-)published our mDNS advertisement. The 518 standard
        # measures first contact "within 10 seconds of advertising", and a
        # fresh re-publish restarts that window.
        self._advertising_since: Optional[float] = None
        ...
        # When the reprobe / mDNS re-register self-heals last ran (None =
        # not yet this silent period). Timestamps rather than one-shot
        # flags: while direct stays dead they repeat every
        # _SILENT_RETRY_INTERVAL_S to keep courting the low-latency path.
        self._last_silent_reprobe: Optional[float] = None
        self._last_silent_rereg: Optional[float] = None
    ...
    def get_diagnostics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            # Split of the same stream by source. `packets_via_router` > 0
            # means the 518 fallback is doing the work VRChat's dead mDNS
            # listener can no longer do; `direct_silent_for_s` is what the
            # fallback ladder actually watches.
            "packets_direct": self._packets_direct,
            "packets_via_router": self._packets_via_router,
            "direct_silent_for_s": self.seconds_since_direct_packet(),
    ...
    def _handle_disconnect(self, reason: str) -> None:
        ...
        self._last_silent_reprobe = None
    ...
    def _emit_diagnostics_tick(self) -> None:
        ...
        # Judged on the DIRECT stream, not on everything we received: once
        # the 518 fallback kicks in, router traffic keeps `last_packet_age_s`
        # fresh, and treating that as health would stop us from ever trying
        # to restore real discovery. The 518 standard wants exactly that —
        # "keep the Tier 0 advertisement alive" while Tier 1 carries data.
        # With no router in play direct == all, so this is unchanged.
        silent_for = self.seconds_since_direct_packet()
        if silent_for is None:
            return
        direct_delta = self._packets_direct - self._direct_at_last_log
        self._direct_at_last_log = self._packets_direct
        if direct_delta == 0 and silent_for >= self._SILENT_THRESHOLD_S:
            if not self._silent_warning_logged:
                self._silent_warning_logged = True
                self._log(
                    f"WARNING: VRChat says connected but we've received "
                    f"0 packets DIRECTLY from it in the last {silent_for:.1f}s"
                    + (f" (the 518 router fallback is carrying "
                       f"{self._packets_via_router} packets meanwhile)"
                       if self._packets_via_router else "")
                    + ". Likely causes: VRChat's mDNS listener has died "
                    f"(restart VRChat to fix), "
                    f"or another OSC app is advertising and VRChat routes to "
                    f"it instead. Try: VRChat → Settings → OSC → Reset."
                )
            def retry_due(last: Optional[float]) -> bool:
                return last is None or (now - last) >= self._SILENT_RETRY_INTERVAL_S

            if silent_for >= self._SILENT_REPROBE_S and retry_due(self._last_silent_reprobe):
                self._last_silent_reprobe = now
            ...
            if silent_for >= self._SILENT_REREGISTER_S and retry_due(self._last_silent_rereg):
                self._last_silent_rereg = now
                self._log(
                    "direct stream still dead — re-publishing mDNS under a fresh "
                    "name to bust VRChat's OSCQuery client cache (retries every "
                    f"{int(self._SILENT_RETRY_INTERVAL_S)}s until direct resumes)"
                )
                threading.Thread(
        ...
        elif direct_delta > 0 and self._silent_warning_logged:
            # The DIRECT stream is back — log the recovery and re-arm the
            # self-heals. Keyed on direct_delta, matching the warning it
            # clears: router-carried traffic must neither clear the latch
            # nor reset the retry clocks (it kept flowing the whole time).
            self._log(f"direct packet flow restored — received {direct_delta} "
                      f"directly in the last {int(self._DIAG_LOG_INTERVAL_S)}s")
            self._silent_warning_logged = False
            self._last_silent_reprobe = None
            self._last_silent_rereg = None
    ...
    def _fetch_osc_ports(self):
        try:
            ...
            if response.status_code == 200:
                ...
                # Our advertisement goes live with this session; the 518
                # fallback's first-contact window starts counting here.
                self._advertising_since = self._connected_since
                self._silent_warning_logged = False
                self._last_silent_reprobe = None
                self._last_silent_rereg = None
    ...
    def _setup_osc(self):
        ...
        # Re-assert the CLASSIFYING wrapper, not the bare handler:
        # set_default_handler replaces rather than chains, so registering
        # _handle_incoming_osc here would silently drop the source-address
        # split — every packet would count as router-carried, the pill would
        # read "CONNECTED (518)" with no router running, and the silent
        # watchdog would re-publish mDNS on a perfectly healthy link.
        self.dispatcher.set_default_handler(self._dispatch_incoming,
                                            needs_reply_address=True)
    ...
    def reregister_mdns(self) -> bool:
        ...
        try:
            ...
            time.sleep(0.5)
            self._advertise_service()
            # A fresh advertisement restarts the 518 standard's first-contact
            # window: VRChat gets another 10 s to discover us before we fall
            # back to the router.
            self._advertising_since = time.time()
            return True
        except Exception as e:
    ...
    def _dispatch_incoming(self, client_address, address: str, *args):
        """Default-handler entry point: classify the datagram's source, then
        delegate to the real handler.

        OSC Router 518 forwards VRChat's stream verbatim from its own socket
        (127.0.0.1:18518), so the source address is the only thing that
        distinguishes a re-broadcast from VRChat pushing to us directly.
        The split drives the 518 fallback ladder; everything downstream of
        here treats both identically, which is correct — the payload is
        byte-for-byte what VRChat sent."""
        via_router = False
        try:
            if (isinstance(client_address, (tuple, list))
                    and len(client_address) >= 2
                    and str(client_address[0]) in _LOOPBACK_HOSTS):
                via_router = int(client_address[1]) == _ROUTER_518_PORT
        except (TypeError, ValueError):
            via_router = False
        if via_router:
            self._packets_via_router += 1
        else:
            self._packets_direct += 1
            self._last_direct_packet_time = time.time()
        return self._handle_incoming_osc(address, *args)

    def seconds_since_direct_packet(self) -> Optional[float]:
        """Seconds since VRChat last pushed to us DIRECTLY — router
        re-broadcasts deliberately don't count. While no direct packet has
        arrived yet, this is the age of the current advertisement (what the
        518 standard's "within 10 seconds of advertising" measures against).
        None when we aren't advertising at all, i.e. there is nothing to
        fall back from."""
        anchor = self._last_direct_packet_time
        if anchor is None:
            anchor = self._advertising_since or self._connected_since
        if anchor is None:
            return None
        return max(0.0, time.time() - anchor)

    def get_flow_state(self) -> OscFlowState:
        """Snapshot for the 518 fallback ladder (osc_router_518)."""
        return OscFlowState(
            port=self.local_listen_port or None,
            advertising=bool(self.is_connected and self.local_listen_port),
            ever_direct=self._last_direct_packet_time is not None,
            since_direct_s=self.seconds_since_direct_packet(),
        )

    def get_link_state(self) -> str:
        """UI-facing link classification (osc_link_state). Raw packet ages,
        not seconds_since_direct_packet(): that helper substitutes the
        advertisement age while no packet has arrived, which is what the
        fallback ladder needs but would show a silent link as live here."""
        now = time.time()
        return link_state(
            connected=bool(self.is_connected),
            since_any_s=(now - self._last_packet_time
                         if self._last_packet_time is not None else None),
            since_direct_s=(now - self._last_direct_packet_time
                            if self._last_direct_packet_time is not None else None),
        )
