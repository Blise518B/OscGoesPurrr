...
class DeviceFrameMixin:
    ...
    def _create_device_frame(self, device_name: str, is_connected: bool,
        ...
        # The toy is a GROUP FRAME in its identity hue (assigned by display
        # position in _reorder_device_frames); the bar below is its filled
        # header, inset 4px so its rounded fill sits inside the frame.
        card = QFrame()
        ...
        return {
            "frame": card,
            # Lets the Overview open this card from outside (focus_device).
    ...
    def focus_device(self, device_name: str) -> None:
        """Open one toy in Device Routing: switch to the page, expand the
        toy's card and scroll it to the top. What an Overview toy tile does
        when clicked. Other cards keep whatever state they were in."""
        self.select_view("Device Routing")
        data = self.device_ui_frames.get(device_name)
        if not data:
            return
        expand = data.get("set_expanded")
        if callable(expand):
            expand(True)
        ...
        def _scroll_to_card():
            # After the expansion has been laid out -- before that, the
            # list's height (and the card's position) is still the old one.
            # The toy list scrolls inside its own card, not with the page,
            # so move the NEAREST scroll area that holds the card.
            try:
                w = frame.parentWidget()
                while w is not None and not isinstance(w, QScrollArea):
                    w = w.parentWidget()
                if w is None or w.widget() is None:
                    return
                top = frame.mapTo(w.widget(), frame.rect().topLeft()).y()
                w.verticalScrollBar().setValue(max(0, top - 6))
            except RuntimeError:
    ...
    def build_device_list_ui(self, devices_dict: dict):
        ...
        if hasattr(self, "rebuild_overview"):
            self.rebuild_overview()
        # The page-level overview bar lists targets from the wrapper
        # registry rebuilt above.
        if hasattr(self, "_refresh_overview_targets"):
    ...
    def _reorder_device_frames(self):
        ...
        for position, name in enumerate(all_names):
            ...
            stretch_index += 1
            # Identity hue by display position: three toys stacked read as
            # three, a fourth starts the cycle over. The bar carries the
            # property too because QSS attribute selectors match the
            # widget they are on, not its ancestors.
            hue = _theme.toy_hue(position)
