...
class DiagnosticsMixin:
    ...
    def _build_network_debug_view(self, parent_layout: QVBoxLayout):
        ...
        sps_header.setObjectName("sectionTitle")
        sps_hdr_row.addWidget(sps_header)
        self._explain(sps_hdr_row,
            "SPS zones",
    ...
    def _build_osc_diagnostics_view(self, parent_layout: QVBoxLayout):
        ...
        intro = QLabel(
            ...
            "Read 'Phonebook GETs received' first: if it is 0, VRChat never "
            "fetched our parameter list, meaning it never discovered us at "
            "all — after a few hours of uptime VRChat's mDNS listener dies "
            "and stops finding newly started OSC apps (restarting VRChat is "
            "the only real fix). If Phonebook GETs is non-zero "
            "but packets stay at 0, another OSC app (VRCOSC, OscGoesBrrr, "
            "etc.) is being routed to instead — those show up in the "
            "'Other OSCQuery clients seen' card below."
        )
        ...
        ports_header.setObjectName("sectionTitle")
        ...
        clients_header.setObjectName("sectionTitle")
        ...
        events_header.setObjectName("sectionTitle")
    ...
    def refresh_osc_diagnostics_view(self) -> None:
        ...
        # 518 fallback ladder: Tier 1 means VRChat can no longer reach us
        # directly and the local router is carrying the stream instead.
        fallback = diag.get("router518") or {}
        via_router = bool(fallback.get("using_router"))
        direct_silent = diag.get("direct_silent_for_s")

        # --- Banner ---------------------------------------------------
        if not is_conn:
            self.diag_status_banner.setText("✕  DISCONNECTED")
            self.diag_status_banner.setStyleSheet(f"color: {COLOR_ALERT};")
        elif via_router and last_age is not None and last_age <= 15.0:
            # Data is flowing, just not from VRChat directly — that's the
            # fallback doing its job, so it reads as OK, not as a fault.
            silent_txt = (f"{direct_silent:.0f}s" if isinstance(direct_silent, (int, float))
                          else "a while")
            self.diag_status_banner.setText(
                f"✓  RECEIVING via OSC Router 518 (Tier 1) — "
                f"VRChat hasn't reached us directly for {silent_txt}"
            )
            self.diag_status_banner.setStyleSheet(f"color: {COLOR_WARNING};")
            ...
            self.diag_status_banner.setText(
                f"⚠  CONNECTED but SILENT for {last_age:.1f}s — "
                f"VRChat is not sending us packets"
            )
            self.diag_status_banner.setStyleSheet(f"color: {COLOR_WARNING};")
        else:
