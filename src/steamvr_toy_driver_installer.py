...
#   * `src\steamvr_toy_driver\`, next to this module           (dev)
...
# The static driver tree (manifest, DLL, vrresources, default settings) —
# laid out by install() and compared by needs_update().
_DRIVER_FILES = (
    ...
    Path("resources") / "input" / "oscgoespurrr_toy_profile.json",
)
...
def steamvr_present() -> bool:
    """SteamVR has run on this PC: it writes openvrpaths.vrpath on its first
    start. Without that file there is nothing to register with — the
    feature is on by default, so a PC without SteamVR must stay untouched."""
    return is_supported() and openvrpaths_file().is_file()
...
def needs_update() -> bool:
    """True when the installed driver differs from the one this build
    carries — an app update brought a new DLL, manifest or settings file,
    or a file is missing. Callers re-run install() to lay the new one down
    (SteamVR picks it up on its next start)."""
    src_root = _bundle_root()
    dst_root = driver_install_root()
    for relpath in _DRIVER_FILES:
