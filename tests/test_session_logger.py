...
class TestMotorEvents:
    ...
    def test_motor_event_shape(self, tmp_path):
        ...
        # The strength/calibration-independent twin of `out`.
        assert e["contact"] == pytest.approx(0.7123)

    def test_motor_contact_defaults_when_omitted(self, tmp_path):
        """The six-field call still works — `contact` is optional so an
        older caller can't crash the logger."""
        logger = SessionLogger(tmp_path)
        sid = logger.start({})
        logger.log_motor(0, "d", 0, 0.5, 0, 0.5, 0, 0.5, 0.4)
        logger.stop()
        e = _events_of(_read_lines(_session_file(tmp_path, sid)), "motor")[0]
        assert e["contact"] == 0.0
