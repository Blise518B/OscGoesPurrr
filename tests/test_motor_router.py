...
class TestCompileMotorConfig:
    ...
    def test_absent_zones_key_means_all_sps(self, router):
        """A motor nobody has configured listens to everything, because
        that is what the zone picker already claims it does.

        The two defaults disagreed: the picker showed "All SPS" while the
        router read "" and applied no zone filter, so the motor was
        silent. Toggling any zone wrote the picker's displayed default
        back out and it started working -- which made the bug look like
        "All SPS only counts if something else is selected too"."""
        cfg = self._config_with()
        compiled = router._compile_motor_config({}, "dev", 0, cfg)
        assert compiled["is_all_sps"] is True
        assert compiled["has_zone_filter"] is True

    def test_explicitly_empty_zones_still_means_none(self, router):
        """Deselecting everything is a real choice and stays silent --
        only an ABSENT key falls back to All SPS."""
        cfg = self._config_with(motor_0_zones="")
        compiled = router._compile_motor_config({}, "dev", 0, cfg)
...
class TestZeroCut:
    ...
    # --- the cut is input-SOURCE agnostic --------------------------------
    # The stage reads `chain_d_raw`, the single max-wins aggregate every
    # input kind feeds. The tests above all drive it through a custom
    # address; these drive the other two ways a motor can be fed, so the
    # SPS half of the stage is covered by something other than inspection.

    def _zone_cfg(self, threshold=0.0):
        cfg = self._cfg(threshold=threshold)
        cfg.pop("osc_addresses", None)
        cfg["motor_0_zones"] = "Boob"
        return cfg

    def test_cuts_an_ogb_zone_contribution(self, router, clock):
        # Touch contact through an OGB zone, then the hand leaves: the cut
        # must fire on a zone-fed d_raw exactly as on a custom address.
        cfg = self._zone_cfg()
        zones = {("Orf", "Boob")}
        params = {"OGB/Orf/Boob/TouchOthersClose": True,
                  "OGB/Orf/Boob/TouchOthers": 1.0}
        router._calculate_motor_target("dev", 0, cfg, params, zones=zones)
        clock.advance(0.05)
        params["OGB/Orf/Boob/TouchOthers"] = 0.0
        out = router._calculate_motor_target("dev", 0, cfg, params, zones=zones)
        assert out == 0.0

    def test_cuts_when_the_new_pen_penetrator_backs_out(self, router, clock):
        # SPS/new-pen path: while root/tip say a penetrator is nearby but
        # not inserted, _get_new_pen_amount reports 0.0 rather than falling
        # back to legacy — and the cut has to act on that 0.
        cfg = self._zone_cfg()
        zones = {("Orf", "Boob")}
        hot = {"OGB/Orf/Boob/TouchOthersClose": True,
               "OGB/Orf/Boob/TouchOthers": 1.0}
        router._calculate_motor_target("dev", 0, cfg, hot, zones=zones)
        clock.advance(0.05)
        # Touch gone; a new-style penetrator is near (root > 0) but its tip
        # is not seated (tip <= 0.99), so the depth model reports 0.0.
        backed_out = {"OGB/Orf/Boob/TouchOthersClose": False,
                      "OGB/Orf/Boob/PenOthersNewRoot": 0.4,
                      "OGB/Orf/Boob/PenOthersNewTip": 0.5}
        out = router._calculate_motor_target("dev", 0, cfg, backed_out,
                                             zones=zones)
        assert out == 0.0

    def test_cuts_a_synthetic_sps_source(self, router, clock):
        # A synthetic source is selected by name in the same zone filter;
        # closing its activation gate drops it to 0 and the cut must fire.
        cfg = self._cfg()
        cfg.pop("osc_addresses", None)
        cfg["motor_0_zones"] = "G-Spot"
        sources = {"G-Spot": {"name": "G-Spot", "zone_type": "Orf",
                              "proximity": ["C/prox"],
                              "activation": ["C/gate"],
                              "velocity": [], "multiplier": 1.0,
                              "max_value": 1.0, "enabled": True}}
        params = {"C/prox": 1.0, "C/gate": True}
    ...
    def test_cut_needs_every_mapped_input_low(self, router, clock):
        # d_raw is max-wins across sources, so the cut is a statement about
        # the motor's whole input, not any one source: a zone going quiet
        # while a mapped custom address stays hot must NOT cut.
        # fall_ms=0 so the output tracks the input directly — a decaying
        # tail would look the same as "didn't cut" for the first few ticks.
        cfg = self._cfg(fall_ms=0.0)
        cfg["motor_0_zones"] = "Boob"
        zones = {("Orf", "Boob")}
        params = {"P": 0.6,
                  "OGB/Orf/Boob/TouchOthersClose": True,
    ...
    def test_cut_is_silence_not_the_output_floor(self, router, clock):
        # The Output band remaps onto [min, max], so a floored chain could
        # plausibly emit its minimum while "cut". It must not: the cut
        # drives the value under _OUTPUT_SILENCE_EPS, and silence is exempt
        # from the band.
        cfg = self._cfg(enabled=True)
        cfg["mix"]["0"]["chains"][0]["output"] = {
            "gain": 1.0, "min": 0.2, "max": 0.8}
        router._calculate_motor_target("dev", 0, cfg, {"P": 1.0}, zones=set())
        clock.advance(0.05)
        out = router._calculate_motor_target("dev", 0, cfg, {"P": 0.0}, zones=set())
        assert out == 0.0
...
# ================================================= directional speed / punch

class TestDirectionalSpeed:
    """Speed splits into an inward and an outward ring, one gain each
    over a shared curve and decay. The split has to be inaudible until
    the two gains actually differ."""

    def _cfg(self, gain=1.0, gain_out=None, decay_ms=300.0):
        cfg = _basic_motor_cfg(osc_addresses={"0": ["P"]})
        chain = cfg["mix"]["0"]["chains"][0]
        chain["depth"]["gain"] = 0.0          # speed channel only
        chain["speed"] = {"gain": gain, "curve": "linear",
                          "curve_param": 1.0, "decay_ms": decay_ms}
        if gain_out is not None:
            chain["speed"]["gain_out"] = gain_out
        chain["smoothing"] = {"rise_ms": 0.0, "fall_ms": 0.0}
        return cfg

    @staticmethod
    def _sweep(router, cfg, clock, positions, step=0.05):
        """Drive a position sequence, returning the output after each."""
        out = []
        for p in positions:
            out.append(router._calculate_motor_target(
                "dev", 0, cfg, {"P": p}, zones=set()))
            clock.advance(step)
        return out

    def test_shipped_default_omits_gain_out(self):
        """`gain_out` must stay ABSENT from the shipped speed block, not
        be written as 1.0. Absent means "follow gain"; pinning it would
        make every later edit of the inward gain alone silently produce
        an asymmetric chain."""
        speed = MotorRouter.DEFAULT_MIX_CONFIG["chains"][0]["speed"]
        assert "gain_out" not in speed

    def test_absent_gain_out_follows_gain(self, router, clock):
        # The behaviour-preservation contract: a chain stored before the
        # split has no gain_out, and must not change feel.
        state = router._calculate_motor_target(
            "dev", 0, self._cfg(gain=0.8), {"P": 0.0}, zones=set())
        assert state == 0.0
        clock.advance(0.05)
        going_in = router._calculate_motor_target(
            "dev", 0, self._cfg(gain=0.8), {"P": 0.6}, zones=set())
        clock.advance(0.05)
        coming_out = router._calculate_motor_target(
            "dev", 0, self._cfg(gain=0.8), {"P": 0.0}, zones=set())
        # Same speed both ways, no gain_out set -> same output.
        assert going_in == pytest.approx(coming_out)
        assert going_in > 0.0

    def test_equal_gains_match_a_symmetric_chain(self, clock):
        # Explicitly setting gain_out == gain must equal leaving it out.
        moves = [0.0, 0.6, 0.0, 0.5, 0.1]
        r1 = MotorRouter(clock=clock.now)
        implicit = self._sweep(r1, self._cfg(gain=0.9), clock, moves)
        clock.advance(5.0)
        r2 = MotorRouter(clock=clock.now)
        explicit = self._sweep(
            r2, self._cfg(gain=0.9, gain_out=0.9), clock, moves)
        assert implicit == pytest.approx(explicit)

    def test_outward_gain_attenuates_only_the_pull_out(self, router, clock):
        cfg = self._cfg(gain=1.0, gain_out=0.25)
        router._calculate_motor_target("dev", 0, cfg, {"P": 0.0}, zones=set())
        clock.advance(0.05)
        going_in = router._calculate_motor_target(
            "dev", 0, cfg, {"P": 0.6}, zones=set())
        clock.advance(0.05)
        coming_out = router._calculate_motor_target(
            "dev", 0, cfg, {"P": 0.0}, zones=set())
        assert going_in > 0.0
        assert coming_out < going_in

    def test_outward_only_chain_is_silent_going_in(self, router, clock):
        # gain 0 / gain_out 1: the toy reacts to withdrawal alone.
        cfg = self._cfg(gain=0.0, gain_out=1.0)
        router._calculate_motor_target("dev", 0, cfg, {"P": 0.0}, zones=set())
        clock.advance(0.05)
        going_in = router._calculate_motor_target(
            "dev", 0, cfg, {"P": 0.6}, zones=set())
        ...
        coming_out = router._calculate_motor_target(
            "dev", 0, cfg, {"P": 0.0}, zones=set())
        assert going_in == 0.0
        assert coming_out > 0.0

    def test_rings_decay_independently(self, router, clock):
        # An out-stroke must not stomp the in-ring's tail: with the
        # outward half muted, the inward tail keeps ringing through the
        # reversal instead of being reset by it.
        cfg = self._cfg(gain=1.0, gain_out=0.0, decay_ms=1000.0)
        router._calculate_motor_target("dev", 0, cfg, {"P": 0.0}, zones=set())
        clock.advance(0.05)
        peak = router._calculate_motor_target(
            "dev", 0, cfg, {"P": 0.8}, zones=set())
        clock.advance(0.05)
        # Reverse hard. The inward ring should still be ringing down.
        during_pullout = router._calculate_motor_target(
            "dev", 0, cfg, {"P": 0.0}, zones=set())
        assert peak > 0.0
        assert during_pullout > 0.0
        assert during_pullout < peak

    def test_state_carries_both_rings(self, router, clock):
        cfg = self._cfg()
        router._calculate_motor_target("dev", 0, cfg, {"P": 0.0}, zones=set())
        clock.advance(0.05)
        router._calculate_motor_target("dev", 0, cfg, {"P": 0.7}, zones=set())
        cs = router._motor_state[("dev", 0)]["chains"][0]
        assert cs["smoothed_speed_in"] > 0.0
        assert cs["smoothed_speed_out"] == pytest.approx(0.0)

    @pytest.mark.parametrize("junk", [None, "loud", float("nan")])
    def test_junk_gain_out_falls_back_to_the_inward_gain(self, clock, junk):
        r = MotorRouter(clock=clock.now)
        cfg = self._cfg(gain=0.8, gain_out=junk)
        r._calculate_motor_target("dev", 0, cfg, {"P": 0.0}, zones=set())
        clock.advance(0.05)
        r._calculate_motor_target("dev", 0, cfg, {"P": 0.6}, zones=set())
        clock.advance(0.05)
        out = r._calculate_motor_target("dev", 0, cfg, {"P": 0.0}, zones=set())
        assert out > 0.0
...
class TestPunch:
    ...
    def test_default_chain_config_has_punch_off(self):
        ...
        # Both directions off by default — the outward hit especially,
        # since it did not exist before and must be opted into.
        assert p == {"gain": 0.0, "gain_out": 0.0, "decay_ms": 120.0}
    ...
    def test_pull_out_is_not_a_punch_by_default(self, router, clock):
        # The outward half exists now but ships at gain_out 0, so the
        # stage still behaves as rise-only until it is asked otherwise.
...
class TestDirectionalPunch:
    """The outward hit: a fast pull-out strikes its own envelope, muted
    unless the chain opts in with `punch.gain_out`."""

    def _cfg(self, gain=0.0, gain_out=0.0, decay_ms=120.0):
        cfg = _basic_motor_cfg(osc_addresses={"0": ["P"]})
        chain = cfg["mix"]["0"]["chains"][0]
        chain["depth"]["gain"] = 0.0
        chain["speed"] = {"gain": 0.0, "curve": "linear",
                          "curve_param": 1.0, "decay_ms": 300.0}
        chain["punch"] = {"gain": gain, "gain_out": gain_out,
                          "decay_ms": decay_ms}
        chain["smoothing"] = {"rise_ms": 0.0, "fall_ms": 0.0}
        return cfg

    def _thrust(self, router, cfg, clock, a, b, dt=0.1):
        router._calculate_motor_target("dev", 0, cfg, {"P": a}, zones=set())
        clock.advance(dt)
        return router._calculate_motor_target(
            "dev", 0, cfg, {"P": b}, zones=set())

    def test_pull_out_strikes_when_enabled(self, router, clock):
        cfg = self._cfg(gain_out=1.0)
        out = self._thrust(router, cfg, clock, 0.6, 0.0)
        assert out == pytest.approx(1.0)

    def test_outward_only_chain_ignores_the_thrust_in(self, router, clock):
        cfg = self._cfg(gain=0.0, gain_out=1.0)
        assert self._thrust(router, cfg, clock, 0.0, 0.6) == 0.0

    def test_inward_only_chain_ignores_the_pull_out(self, router, clock):
        cfg = self._cfg(gain=1.0, gain_out=0.0)
        assert self._thrust(router, cfg, clock, 0.6, 0.0) == 0.0

    def test_the_two_gains_scale_their_own_direction(self, router, clock):
        cfg = self._cfg(gain=1.0, gain_out=0.4)
        going_in = self._thrust(router, cfg, clock, 0.0, 0.9)
        clock.advance(2.0)      # let both envelopes ring out
        router._calculate_motor_target("dev", 0, cfg, {"P": 0.9}, zones=set())
        clock.advance(0.1)
        coming_out = router._calculate_motor_target(
            "dev", 0, cfg, {"P": 0.0}, zones=set())
        assert going_in == pytest.approx(1.0)
        assert coming_out == pytest.approx(0.4, abs=0.05)

    def test_envelopes_do_not_stomp_each_other(self, router, clock):
        # A thrust is in-then-out. With only the inward half enabled, the
        # out-stroke must leave the inward tail alone.
        cfg = self._cfg(gain=1.0, gain_out=0.0, decay_ms=500.0)
        peak = self._thrust(router, cfg, clock, 0.0, 0.9)
        clock.advance(0.05)
        after_reversal = router._calculate_motor_target(
            "dev", 0, cfg, {"P": 0.0}, zones=set())
        assert peak == pytest.approx(1.0)
        assert after_reversal > 0.5      # still ringing, not reset

    def test_both_gains_zero_skips_and_resets(self, router, clock):
        cfg = self._cfg(gain=0.0, gain_out=0.0)
        assert self._thrust(router, cfg, clock, 0.0, 0.9) == 0.0
        cs = router._motor_state[("dev", 0)]["chains"][0]
        assert cs["punch_env_in"] == 0.0
        assert cs["punch_env_out"] == 0.0
        # Memory still tracked, so enabling mid-motion reads no phantom.
        assert cs["punch_last_d"] == pytest.approx(0.9)

    def test_directional_halves_ride_the_emit_stream(self, router, clock):
        cfg = self._cfg(gain=1.0, gain_out=1.0)
        captured = []
        router.subscribe_intermediates("dev", 0, 0, captured.append)
        self._thrust(router, cfg, clock, 0.6, 0.0)
        last = captured[-1]
        assert last["punch_out"] == pytest.approx(1.0)
        assert last["punch_in"] < last["punch_out"]
        assert last["punch"] == pytest.approx(last["punch_out"])
...
class TestArming:
    ...
    def test_default_wake_config_is_off_activity_mode(self):
        ...
        # Depth-weighted movement by default: a speed-only source is as
        # twitch-sensitive as Punch, which made the gate hair-triggered.
...
class TestTexture:
    ...
    def _cfg(self, enabled=True, amount=0.5, rate_hz=2.5, follow=False,
        ...
        cfg["mix"]["0"]["chains"][0]["texture"] = {
            ...
            "depth_follow": depth_follow,
        }
        return cfg

    def _run_motion(self, router, clock, cfg, ticks=120, dt=1 / 60):
        """Drive a sustained in/out stroke so the speed channel stays hot
        (movement > 0) and return the per-tick outputs. The stroke input
        sequence is identical across modes, so any output difference is
        purely the depth_follow amount modulation. `_basic_motor_cfg`
        ships the speed channel muted (gain 0), so enable it here — without
        a live speed signal `depth_follow` has nothing to track."""
        cfg["mix"]["0"]["chains"][0]["speed"]["gain"] = 1.0
        outs = []
        router._calculate_motor_target("dev", 0, cfg, {"P": 0.2}, zones=set())
        for i in range(ticks):
            clock.advance(dt)
            phase = (i % 8) / 8.0            # triangle stroke, 0.2 .. 1.0
            tri = phase * 2.0 if phase < 0.5 else (1.0 - phase) * 2.0
            outs.append(router._calculate_motor_target(
                "dev", 0, cfg, {"P": 0.2 + 0.8 * tri}, zones=set()))
        return outs
    ...
    def test_depth_follow_is_inert_while_holding_still(self, router, clock):
        # At rest the speed signal is 0, so every depth_follow mode reduces
        # to the same constant-amount wobble — "up"/"down" only bite once
        # there is movement.
        off = self._run_held(router, clock, self._cfg(depth_follow="off"))
        r_down = MotorRouter(clock=clock.now)
        down = self._run_held(r_down, clock, self._cfg(depth_follow="down"))
        r_up = MotorRouter(clock=clock.now)
        up = self._run_held(r_up, clock, self._cfg(depth_follow="up"))
        assert off == pytest.approx(down)
        assert off == pytest.approx(up)

    def test_depth_follow_scales_wobble_with_movement(self, clock):
        # Same stroke input through each mode. Because the input (and thus
        # the level, LFO phase and movement) is identical across modes, the
        # only per-tick difference is the amount modulation: "Weaker" shaves
        # the downward attenuation under motion (higher total), "Stronger"
        # deepens it (lower total). Direction, deterministically.
        def total(mode):
            r = MotorRouter(clock=clock.now)
            return sum(self._run_motion(
                r, clock, self._cfg(amount=0.6, depth_follow=mode)))
        off = total("off")
        weaker = total("down")
        stronger = total("up")
        assert weaker > off > stronger
...
class TestActivityGateIntegration:
    ...
    def _gate_cfg(self, **gate_overrides):
        ...
        gate flips, simplifying assertions. Source pinned to "speed": this
        class validates the meter/gate mechanics (taus, threshold, sleep
        delay) against the speed signal they were calibrated on; source
        selection is TestWakeSource's job."""
        cfg = _basic_motor_cfg(osc_addresses={"0": ["P"]})
        chain = cfg["mix"]["0"]["chains"][0]
        chain["wake"] = {
            "enabled": True,
...
class TestWakeSource:
    """`wake.source` picks what charges the activity meter: raw depth,
    the raw speed detector, or depth × speed ("both", the default). Punch
    must never feed the meter through ANY source — it exaggerates tiny
    movements by design, which made the gate hair-triggered."""

    def _cfg(self, source, **wake_overrides):
        cfg = _basic_motor_cfg(osc_addresses={"0": ["P"]})
        chain = cfg["mix"]["0"]["chains"][0]
        chain["wake"] = {
            "enabled": True,
        ...
        chain["wake"].update(wake_overrides)
        chain["smoothing"] = {"rise_ms": 0.0, "fall_ms": 0.0}
        return cfg

    def _drive(self, router, clock, cfg, values, dt=0.03, device="dev"):
        router._calculate_motor_target(device, 0, cfg, {"P": values[0]}, zones=set())
        for v in values[1:]:
            clock.advance(dt)
            router._calculate_motor_target(device, 0, cfg, {"P": v}, zones=set())
        return router._motor_state[(device, 0)]["chains"][0]

    def test_source_resolver(self):
        assert MotorRouter._wake_source({"source": "depth"}, 0.7, 0.2) == 0.7
        assert MotorRouter._wake_source({"source": "speed"}, 0.7, 0.2) == 0.2
        assert MotorRouter._wake_source({"source": "both"}, 0.5, 0.4) == pytest.approx(0.2)
        # Missing / unknown values fall back to the "both" product.
        assert MotorRouter._wake_source({}, 0.5, 0.4) == pytest.approx(0.2)
        assert MotorRouter._wake_source({"source": "punch"}, 0.5, 0.4) == pytest.approx(0.2)

    def test_depth_source_wakes_on_presence_alone(self, router, clock):
        # Held insertion, zero movement: depth charges the meter, so the
        # gate opens without a single stroke — presence semantics.
        state = self._drive(router, clock, self._cfg("depth"), [0.6] * 30)
        assert state["gate_open"] is True
        # The same stillness leaves a speed-sourced gate shut.
        r2 = MotorRouter(clock=clock.now)
        state2 = self._drive(r2, clock, self._cfg("speed"), [0.6] * 30)
        assert state2["gate_open"] is False

    def test_both_source_ignores_shallow_flutter(self, router, clock):
        # The user-reported failure mode: a tiny fast jiggle near the
        # entrance. Velocity is high (speed source opens), but depth is
        # tiny — the product keeps the meter parked below threshold.
        flutter = [0.0 if i % 2 == 0 else 0.06 for i in range(40)]
        state = self._drive(router, clock, self._cfg("both"), flutter)
        assert state["gate_open"] is False
        r2 = MotorRouter(clock=clock.now)
        state2 = self._drive(r2, clock, self._cfg("speed"), flutter)
        assert state2["gate_open"] is True   # why "speed" isn't the default

    def test_both_source_wakes_on_real_strokes(self, router, clock):
        # Deep in-out strokes: depth AND speed present → product charges
        # the meter and the gate opens.
        strokes = [0.2 if i % 2 == 0 else 0.9 for i in range(40)]
        state = self._drive(router, clock, self._cfg("both"), strokes)
        assert state["gate_open"] is True

    @pytest.mark.parametrize("source", ["depth", "speed", "both"])
    def test_punch_never_feeds_the_wake_meter(self, source):
        # Identical motion with punch fully off vs cranked to the max:
        # the wake meter and gate state must match tick for tick. Punch
        # exaggerating the OUTPUT is fine; exaggerating the GATE was the
        # bug this knob exists to prevent. Each run gets its OWN clock
        # starting at 0 so the comparison is bit-exact — sharing one
        # running clock shifts the absolute times and float rounding of
        # `now - last_time` produces 1e-16 noise that isn't punch's doing.
        motion = [0.0, 0.15, 0.05, 0.2, 0.1, 0.25, 0.05, 0.3] * 5

        def run(punch_gain):
            t = {"now": 0.0}
            r = MotorRouter(clock=lambda: t["now"])
            cfg = self._cfg(source)
            cfg["mix"]["0"]["chains"][0]["punch"] = {
                "gain": punch_gain, "decay_ms": 1000.0}
            meters = []
            r._calculate_motor_target("dev", 0, cfg, {"P": motion[0]}, zones=set())
            for v in motion[1:]:
                t["now"] += 0.03
                r._calculate_motor_target("dev", 0, cfg, {"P": v}, zones=set())
                st = r._motor_state[("dev", 0)]["chains"][0]
                meters.append((st["activity_meter"], st["gate_open"]))
            return meters

        assert run(0.0) == run(2.0)
...
class TestMultiChainRouting:
    ...
    def test_caps_chains_at_the_limit(self, router):
        """A profile asking for more chains than the router accepts is
        silently truncated (the UI enforces the same cap). With every
        chain at gain 1.0 and merge=add, the output reflects exactly
        _MAX_CHAINS_PER_MOTOR of them."""
        chain = {
            "depth": {"gain": 1.0, "curve": "linear", "curve_param": 1.0},
            "speed": {"gain": 0.0, "curve": "linear", "curve_param": 1.0},
            "combine": "max",
        ...
        cap = MotorRouter._MAX_CHAINS_PER_MOTOR
        cfg = _basic_motor_cfg(osc_addresses={"0": ["P"]})
        cfg["mix"]["0"] = {
            "chains": [copy.deepcopy(chain) for _ in range(cap + 2)],
            "merge": "add",
        }
        out = router._calculate_motor_target("dev", 0, cfg, {"P": 0.1},
                                             zones=set())
        assert out == pytest.approx(min(1.0, 0.1 * cap))
        # State is only allocated for the chains that survived the cap.
        state = router._motor_state[("dev", 0)]
        assert len(state["chains"]) == cap
...
class TestAntiStuck:
    ...
    def test_cut_does_not_perturb_speed_detector(self, router, clock):
        ...
        # in either direction.
        assert chain_state["smoothed_speed_in"] == pytest.approx(0.0)
        assert chain_state["smoothed_speed_out"] == pytest.approx(0.0)
...
class TestOutputGain:
    """The per-chain Output gain: hardware calibration applied at the end
    of the chain, so one toy that runs hot can be pulled into line without
    touching the global strength or the chain tuning."""

    def _cfg(self, gain=None):
        cfg = _basic_motor_cfg(osc_addresses={"0": ["P"]})
        if gain is not None:
            cfg["mix"]["0"]["chains"][0]["output"] = {"gain": gain}
        return cfg

    def test_absent_key_leaves_output_untouched(self, router):
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(), {"P": 0.8}, zones=set())
        assert out == pytest.approx(0.8)

    def test_trims_down(self, router):
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(0.5), {"P": 0.8}, zones=set())
        assert out == pytest.approx(0.4)

    def test_zero_silences_the_motor(self, router):
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(0.0), {"P": 1.0}, zones=set())
        assert out == 0.0

    def test_boost_clamps_at_full_output(self, router):
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(2.0), {"P": 0.8}, zones=set())
        assert out == 1.0

    def test_above_the_cap_is_clamped_not_rejected(self, router):
        # A hand-edited profile asking for ×50 gets the ×2 ceiling, not a
        # crash and not a silent fall back to ×1.
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(50.0), {"P": 0.3}, zones=set())
        assert out == pytest.approx(0.6)

    @pytest.mark.parametrize("junk", ["loud", None, float("nan"), -1.0])
    def test_junk_falls_back_to_untrimmed(self, router, junk):
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(junk), {"P": 0.8}, zones=set())
        assert out == pytest.approx(0.8 if junk != -1.0 else 0.0)

    def test_applies_per_chain_before_the_merge(self, router):
        # Each chain carries its own gain, so two half-gained chains
        # merged with `add` sum back to full — the gain is NOT one
        # post-merge multiply (which would give 0.5 here).
        cfg = _basic_motor_cfg(osc_addresses={"0": ["P"]})
        per_motor = cfg["mix"]["0"]
        per_motor["chains"].append(copy.deepcopy(per_motor["chains"][0]))
        per_motor["merge"] = "add"
        for chain in per_motor["chains"]:
            chain["output"] = {"gain": 0.5}
        out = router._calculate_motor_target(
            "dev", 0, cfg, {"P": 1.0}, zones=set())
        assert out == pytest.approx(1.0)


class TestOutputRange:
    """The per-chain Output band: the toy's usable output range. It REMAPS
    the chain's full 0..1 onto [min, max] instead of clipping, so a dead
    low range or a harsh top can be skipped without losing input
    fidelity."""

    def _cfg(self, out_min=None, out_max=None, gain=None):
        cfg = _basic_motor_cfg(osc_addresses={"0": ["P"]})
        band = {}
        ...
        if gain is not None:
            band["gain"] = gain
        cfg["mix"]["0"]["chains"][0]["output"] = band
        return cfg

    def test_full_range_is_inert(self, router):
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(0.0, 1.0), {"P": 0.37}, zones=set())
        assert out == pytest.approx(0.37)

    def test_full_input_lands_on_the_ceiling(self, router):
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(0.2, 0.8), {"P": 1.0}, zones=set())
        assert out == pytest.approx(0.8)

    def test_mid_input_lands_mid_band(self, router):
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(0.2, 0.8), {"P": 0.5}, zones=set())
        assert out == pytest.approx(0.5)

    def test_the_smallest_real_input_jumps_to_the_floor(self, router):
        # 2 % in — over the silence epsilon, so it lands just above the
        # floor rather than in the dead zone the floor exists to skip.
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(0.2, 0.8), {"P": 0.02}, zones=set())
        assert out == pytest.approx(0.2 + 0.02 * 0.6)

    def test_silence_stays_silent(self, router):
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(0.2, 0.8), {"P": 0.0}, zones=set())
        assert out == 0.0

    def test_under_the_epsilon_is_silence_not_the_floor(self, router):
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(0.2, 0.8), {"P": 0.005}, zones=set())
        assert out == 0.0

    def test_gain_is_applied_before_the_band(self, router):
        # ×2 on a half input saturates at 1.0, which the band then maps
        # to its ceiling.
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(0.2, 0.8, gain=2.0), {"P": 0.5}, zones=set())
        assert out == pytest.approx(0.8)

    def test_inverted_band_collapses_to_the_ceiling(self, router):
        # Only reachable by hand-editing; must not emit a negative span.
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(0.9, 0.4), {"P": 0.5}, zones=set())
        assert out == pytest.approx(0.4)

    @pytest.mark.parametrize("junk", ["loud", None, float("nan")])
    def test_junk_band_falls_back_to_full_range(self, router, junk):
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(junk, junk), {"P": 0.8}, zones=set())
        assert out == pytest.approx(0.8)

    def test_merge_never_climbs_past_the_highest_ceiling(self, router):
        # `add` would sum two capped chains to 1.2 → clamped to 1.0 by
        # merge_chains, which is still past the 0.6 ceiling the user set
        # for this hardware. The cap survives the merge.
        cfg = _basic_motor_cfg(osc_addresses={"0": ["P"]})
        per_motor = cfg["mix"]["0"]
        per_motor["chains"].append(copy.deepcopy(per_motor["chains"][0]))
        per_motor["merge"] = "add"
        for chain in per_motor["chains"]:
            chain["output"] = {"min": 0.0, "max": 0.6}
        out = router._calculate_motor_target(
            "dev", 0, cfg, {"P": 1.0}, zones=set())
        assert out == pytest.approx(0.6)


class TestOutputBandVersusStrength:
    """The global strength is folded into the Output stage AHEAD of the
    band — a floor applied before the dispatch multiply would be scaled
    straight back into the toy's dead zone."""

    def _cfg(self, out_min=0.2, out_max=0.8):
        cfg = _basic_motor_cfg(osc_addresses={"0": ["P"]})
        cfg["mix"]["0"]["chains"][0]["output"] = {
    ...
    def test_default_getter_is_unity(self, router):
        # A router built without the getter behaves as strength 1.0.
        out = router._calculate_motor_target(
            "dev", 0, self._cfg(), {"P": 1.0}, zones=set())
        assert out == pytest.approx(0.8)

    def test_strength_rides_inside_the_band(self, clock):
        r = MotorRouter(clock=clock.now, get_master_scale=lambda: 0.5)
        out = r._calculate_motor_target(
            "dev", 0, self._cfg(), {"P": 1.0}, zones=set())
        # Half strength lands halfway up the band, NOT at half the
        # ceiling (0.4) — the floor still holds.
        assert out == pytest.approx(0.5)

    def test_a_quiet_strength_cannot_push_under_the_floor(self, clock):
        r = MotorRouter(clock=clock.now, get_master_scale=lambda: 0.05)
        out = r._calculate_motor_target(
            "dev", 0, self._cfg(), {"P": 1.0}, zones=set())
        assert out == pytest.approx(0.2 + 0.05 * 0.6)

    def test_off_is_silent_through_a_floor(self, clock):
        r = MotorRouter(clock=clock.now, get_master_scale=lambda: 0.0)
        out = r._calculate_motor_target(
            "dev", 0, self._cfg(), {"P": 1.0}, zones=set())
        assert out == 0.0

    def test_scales_an_unbanded_chain_too(self, clock):
        # Dispatch no longer multiplies, so the plain case has to carry
        # the strength as well.
        r = MotorRouter(clock=clock.now, get_master_scale=lambda: 0.5)
        out = r._calculate_motor_target(
            "dev", 0, _basic_motor_cfg(osc_addresses={"0": ["P"]}),
            {"P": 0.8}, zones=set())
        assert out == pytest.approx(0.4)

    def test_a_throwing_getter_does_not_silence_the_rig(self, clock):
        def boom():
            raise RuntimeError("nope")
        r = MotorRouter(clock=clock.now, get_master_scale=boom)
        out = r._calculate_motor_target(
            "dev", 0, _basic_motor_cfg(osc_addresses={"0": ["P"]}),
            {"P": 0.8}, zones=set())
        assert out == pytest.approx(0.8)

class TestContactOutputs:
    """The param-out mirror's feed: the same chain stopped BEFORE the
    Output stage, so a mirrored avatar parameter reports the contact the
    chain detected rather than the drive the toy received. Debounced
    separately from last_outputs, which moves independently of it."""

    def _cfg(self, out_cfg=None):
        cfg = _basic_motor_cfg(osc_addresses={"0": ["P"]})
        cfg["mix"]["0"]["chains"][0]["output"] = out_cfg or {
            "gain": 1.0, "min": 0.2, "max": 0.8}
        return cfg

    def test_contact_ignores_the_band_and_the_gain(self, clock):
        r = MotorRouter(clock=clock.now)
        toy = r._calculate_motor_target(
            "dev", 0, self._cfg({"gain": 0.5, "min": 0.2, "max": 0.8}),
            {"P": 1.0}, zones=set())
        # The toy gets the trimmed value inside the band…
        assert toy == pytest.approx(0.2 + 0.5 * 0.6)
        # …the mirror gets the contact, untouched by either.
        assert r.last_contact_outputs[("dev", 0)] == pytest.approx(1.0)

    def test_contact_ignores_the_strength(self, clock):
        r = MotorRouter(clock=clock.now, get_master_scale=lambda: 0.25)
        r._calculate_motor_target("dev", 0, self._cfg(), {"P": 0.8},
                                  zones=set())
        assert r.last_contact_outputs[("dev", 0)] == pytest.approx(0.8)

    def test_off_silences_the_toy_but_not_the_mirror(self, clock):
        # The sharpest case: Off is a TOY panic switch. An avatar visual
        # must not start claiming there is no contact.
        r = MotorRouter(clock=clock.now, get_master_scale=lambda: 0.0)
        toy = r._calculate_motor_target("dev", 0, self._cfg(), {"P": 0.7},
                                        zones=set())
        assert toy == 0.0
        assert r.last_contact_outputs[("dev", 0)] == pytest.approx(0.7)

    def test_contact_updates_fire_while_the_toy_value_is_pinned(self, clock):
        # In Off every toy value is 0, so reevaluate_state emits no updates
        # after the first — the mirror needs its own change stream or it
        # freezes exactly when the panic switch is on.
        r = MotorRouter(clock=clock.now, get_master_scale=lambda: 0.0)
        profile = {"dev": {**self._cfg(), "motor_count": 1}}
        r.reevaluate_state(profile, {"P": 0.3}, zones=set())
        r.consume_contact_updates()
        clock.advance(0.05)
        toy_updates = r.reevaluate_state(profile, {"P": 0.9}, zones=set())
        contact_updates = r.consume_contact_updates()
        assert toy_updates == []          # toy pinned at 0, nothing to send
        assert contact_updates == [("dev", 0, pytest.approx(0.9))]

    def test_zero_cut_reaches_the_mirror(self, clock):
        # Everything upstream of Output still shapes the contact.
        cfg = self._cfg()
        cfg["mix"]["0"]["chains"][0]["zerocut"] = {
            "enabled": True, "threshold": 0.0}
        cfg["mix"]["0"]["chains"][0]["smoothing"] = {
            "rise_ms": 0.0, "fall_ms": 500.0}
        r = MotorRouter(clock=clock.now)
        r._calculate_motor_target("dev", 0, cfg, {"P": 1.0}, zones=set())
        clock.advance(0.05)
        r._calculate_motor_target("dev", 0, cfg, {"P": 0.0}, zones=set())
        assert r.last_contact_outputs[("dev", 0)] == 0.0

    def test_consume_drains(self, clock):
        r = MotorRouter(clock=clock.now)
        r._calculate_motor_target("dev", 0, self._cfg(), {"P": 0.5},
                                  zones=set())
        assert len(r.consume_contact_updates()) == 1
        assert r.consume_contact_updates() == []

    def test_forget_device_drops_contact_state(self, clock):
        r = MotorRouter(clock=clock.now)
        r._calculate_motor_target("dev", 0, self._cfg(), {"P": 0.5},
                                  zones=set())
        r.forget_device("dev")
        assert ("dev", 0) not in r.last_contact_outputs


class TestOutputBandMapping:
    """`map_into_output_band` — for callers that inject a level of their
    own (the OGP/Test connectivity pulse) rather than routing one. A fixed
    raw level stops being comparable across a calibrated rig."""

    def _cfg(self, out_cfg):
        cfg = _basic_motor_cfg(osc_addresses={"0": ["P"]})
        cfg["mix"]["0"]["chains"][0]["output"] = out_cfg
        return cfg

    def _routed(self, clock, out_cfg):
        r = MotorRouter(clock=clock.now)
        r._calculate_motor_target("dev", 0, self._cfg(out_cfg), {"P": 0.5},
                                  zones=set())
        return r

    def test_unrouted_motor_passes_the_level_through(self, clock):
        # Nothing known about the motor yet — the pre-band behaviour.
        r = MotorRouter(clock=clock.now)
        assert r.map_into_output_band("dev", 0, 0.2) == pytest.approx(0.2)

    def test_inert_band_is_a_no_op(self, clock):
        r = self._routed(clock, {"gain": 1.0, "min": 0.0, "max": 1.0})
        assert r.map_into_output_band("dev", 0, 0.2) == pytest.approx(0.2)

    def test_pulse_lands_inside_a_raised_floor(self, clock):
        # The whole point: a 20 % pulse on a motor that does nothing below
        # 30 % must not sit in the dead zone.
        r = self._routed(clock, {"gain": 1.0, "min": 0.3, "max": 1.0})
        mapped = r.map_into_output_band("dev", 0, 0.2)
        assert mapped == pytest.approx(0.3 + 0.2 * 0.7)
        assert mapped > 0.3

    def test_pulse_respects_the_ceiling(self, clock):
        r = self._routed(clock, {"gain": 1.0, "min": 0.2, "max": 0.5})
        assert r.map_into_output_band("dev", 0, 1.0) == pytest.approx(0.5)

    def test_zero_stays_zero(self, clock):
        # The floor is where the toy starts moving, not a level it always
        # emits — a released pulse must not leave the motor humming.
        r = self._routed(clock, {"gain": 1.0, "min": 0.3, "max": 1.0})
        assert r.map_into_output_band("dev", 0, 0.0) == 0.0

    @pytest.mark.parametrize("junk", [None, "loud", float("nan"),
                                      float("inf")])
    def test_junk_level_is_silence_not_a_crash(self, clock, junk):
        r = self._routed(clock, {"gain": 1.0, "min": 0.3, "max": 1.0})
        assert r.map_into_output_band("dev", 0, junk) == 0.0

    def test_floor_takes_the_highest_of_two_chains(self, clock):
        # The band describes the motor, so an uncalibrated sibling chain
        # sitting at 0 must not drag the pulse back into the dead zone.
        import copy as _copy
        cfg = self._cfg({"gain": 1.0, "min": 0.4, "max": 1.0})
        chain1 = _copy.deepcopy(cfg["mix"]["0"]["chains"][0])
        chain1["output"] = {"gain": 1.0, "min": 0.0, "max": 1.0}
        cfg["mix"]["0"]["chains"].append(chain1)
        r = MotorRouter(clock=clock.now)
        r._calculate_motor_target("dev", 0, cfg, {"P": 0.5}, zones=set())
        assert r.last_output_bands[("dev", 0)] == (0.4, 1.0)

    def test_band_survives_a_mode_switch(self, clock):
        # reset_outputs clears routing state; the band is hardware, so the
        # pulse must stay calibrated across a mode change.
        r = self._routed(clock, {"gain": 1.0, "min": 0.3, "max": 1.0})
        r.reset_outputs()
        assert r.map_into_output_band("dev", 0, 0.2) > 0.3

    def test_forget_device_drops_the_band(self, clock):
        r = self._routed(clock, {"gain": 1.0, "min": 0.3, "max": 1.0})
        r.forget_device("dev")
        assert r.map_into_output_band("dev", 0, 0.2) == pytest.approx(0.2)


class TestSleepWakeOverride:
    """The Sleep toggle hands every chain's Wake stage over to the stroke
    counter for the session, without writing to the stored chain."""

    def _cfg(self):
        cfg = _basic_motor_cfg(osc_addresses={"0": ["P"]})
        # Wake explicitly OFF in the stored chain — so anything that gates
        # the output below can only have come from the Sleep override.
        cfg["mix"]["0"]["chains"][0]["wake"] = {"enabled": False}
        return cfg

    def test_awake_router_passes_signal_through(self, clock):
        r = MotorRouter(clock=clock.now)
        out = r._calculate_motor_target(
            "dev", 0, self._cfg(), {"P": 0.8}, zones=set())
        assert out == pytest.approx(0.8)

    def test_sleeping_router_gates_the_signal(self, clock):
        r = MotorRouter(clock=clock.now, get_sleep_active=lambda: True)
        out = r._calculate_motor_target(
            "dev", 0, self._cfg(), {"P": 0.8}, zones=set())
        # Stroke counter hasn't seen its three strokes yet.
        assert out == 0.0

    def test_strokes_open_the_gate(self, clock):
        r = MotorRouter(clock=clock.now, get_sleep_active=lambda: True)
        cfg = self._cfg()
        for _ in range(4):      # four full in-out strokes, well inside 6 s
            for depth in (1.0, 0.0):
                r._calculate_motor_target("dev", 0, cfg, {"P": depth},
                                          zones=set())
                clock.advance(0.2)
        out = r._calculate_motor_target("dev", 0, cfg, {"P": 0.8},
                                        zones=set())
        assert out > 0.0

    def test_stored_wake_config_is_never_written(self, clock):
        r = MotorRouter(clock=clock.now, get_sleep_active=lambda: True)
        cfg = self._cfg()
        r._calculate_motor_target("dev", 0, cfg, {"P": 0.8}, zones=set())
        assert cfg["mix"]["0"]["chains"][0]["wake"] == {"enabled": False}

    def test_toggling_off_restores_the_stored_wake_settings(self, clock):
        sleeping = {"on": True}
        r = MotorRouter(clock=clock.now,
                        get_sleep_active=lambda: sleeping["on"])
        cfg = self._cfg()
        assert r._calculate_motor_target(
            "dev", 0, cfg, {"P": 0.8}, zones=set()) == 0.0
        sleeping["on"] = False
        clock.advance(0.05)
        out = r._calculate_motor_target("dev", 0, cfg, {"P": 0.8},
    ...
    def test_toggling_resets_wake_state(self, clock):
        # Waking the stroke counter then leaving Sleep and re-entering it
        # must NOT stay open on the old counter's credit.
        sleeping = {"on": True}
        r = MotorRouter(clock=clock.now,
                        get_sleep_active=lambda: sleeping["on"])
        cfg = self._cfg()
    ...
    def test_getter_failure_is_treated_as_awake(self, clock):
        def boom():
            raise RuntimeError("controller went away mid-teardown")
        r = MotorRouter(clock=clock.now, get_sleep_active=boom)
        out = r._calculate_motor_target(
            "dev", 0, self._cfg(), {"P": 0.8}, zones=set())
        assert out == pytest.approx(0.8)

    def test_defaults_to_awake_without_a_getter(self, clock):
        r = MotorRouter(clock=clock.now)
        assert r.get_sleep_active() is False
...
# ================================================ typed chains: touch vs pen

class TestChainTypes:
    """Touch and penetration get a chain each, because they are different
    sensations that want different tuning. A chain's TYPE decides which
    contact reaches it; zones and self/others stay per-chain choices."""

    def _cfg(self, chains):
        cfg = _basic_motor_cfg()
        cfg["mix"] = {"0": {"chains": chains, "merge": "max"}}
        return cfg

    @staticmethod
    def _chain(ctype=None, **routing):
        c = {
            "depth": {"gain": 1.0, "curve": "linear", "curve_param": 1.0},
            "speed": {"gain": 0.0, "curve": "linear", "curve_param": 1.0},
            "combine": "max",
    ...
    # Touch on Boob, penetration on Boob — both present at once, so a
    # chain's output tells us exactly which one it responded to.
    PARAMS = {
        "OGB/Orf/Boob/TouchOthersClose": True,
        "OGB/Orf/Boob/TouchOthers": 0.9,
        "OGB/Orf/Boob/PenOthers": 0.4,
    }
    ZONES = {("Orf", "Boob")}

    def _chain_outs(self, router, cfg):
        captured = []
        for idx in range(len(cfg["mix"]["0"]["chains"])):
            router.subscribe_intermediates("dev", 0, idx, captured.append)
        router._calculate_motor_target("dev", 0, cfg, self.PARAMS,
                                       zones=self.ZONES)
        return [c["d_raw"] for c in captured]

    def test_touch_chain_sees_only_touch(self, router):
        cfg = self._cfg([self._chain("touch")])
        assert self._chain_outs(router, cfg) == [pytest.approx(0.9)]

    def test_penetration_chain_sees_only_penetration(self, router):
        cfg = self._cfg([self._chain("penetration")])
        assert self._chain_outs(router, cfg) == [pytest.approx(0.4)]

    def test_the_two_chains_resolve_different_input(self, router):
        """The whole point of the split: one motor, two chains, each fed
        by its own kind of contact rather than one merged compromise."""
        cfg = self._cfg([self._chain("penetration"), self._chain("touch")])
        pen, touch = self._chain_outs(router, cfg)
        assert pen == pytest.approx(0.4)
        assert touch == pytest.approx(0.9)

    def test_untyped_chain_behaves_as_before(self, router):
        """No `type` means custom, which honours the explicit filters —
        exactly how every chain worked before types existed. Nothing
        changes feel on its own."""
        cfg = self._cfg([self._chain()])
        # Both kinds of contact reach it, so it sees the louder one.
        assert self._chain_outs(router, cfg) == [pytest.approx(0.9)]

    def test_custom_chain_honours_explicit_filters(self, router):
        cfg = self._cfg([self._chain("custom")])
        cfg["motor_0_touch"] = False        # penetration only, by hand
        assert self._chain_outs(router, cfg) == [pytest.approx(0.4)]

    def test_per_chain_zones_override_the_motor(self, router):
        """A chain with its own zone selection ignores the motor's."""
        cfg = self._cfg([self._chain("touch"), self._chain("touch")])
        cfg["motor_0_zones"] = "All SPS"
        cfg["motor_0_c1_zones"] = "Tail"        # chain 1 looks elsewhere
        first, second = self._chain_outs(router, cfg)
        assert first == pytest.approx(0.9)
        assert second == 0.0

    def test_per_chain_self_others_override_the_motor(self, router):
        cfg = self._cfg([self._chain("touch"), self._chain("touch")])
        cfg["motor_0_c1_others"] = False        # chain 1 ignores others
        first, second = self._chain_outs(router, cfg)
        assert first == pytest.approx(0.9)
        assert second == 0.0

    def test_a_chain_without_its_own_keys_falls_back_to_the_motor(self, router):
        """Chains that have never been given routing keys behave exactly
        as the motor did before it was split."""
        cfg = self._cfg([self._chain("touch")])
        cfg["motor_0_zones"] = "Tail"           # motor-level only
        assert self._chain_outs(router, cfg) == [0.0]

    def test_motor_level_d_raw_is_the_max_across_chains(self, router):
        """live_d_raw drives the stroke counter, the anti-stuck fuse and
        the usage stats, which are statements about the MOTOR."""
        cfg = self._cfg([self._chain("penetration"), self._chain("touch")])
        out = router._calculate_motor_target("dev", 0, cfg, self.PARAMS,
                                             zones=self.ZONES)
        # merge=max over two chains: the louder (touch, 0.9) wins.
        assert out == pytest.approx(0.9)

    def test_chain_routing_key_shape_stays_mode_local(self):
        """Per-chain keys must still classify as routing, or they would
        silently become shared across all four modes."""
        from config_manager import is_routing_key
        from motor_router import MotorRouter
        for suffix in ("zones", "touch", "pen", "self", "others"):
            key = MotorRouter.chain_routing_key(0, 1, suffix)
            assert key == f"motor_0_c1_{suffix}"
            assert is_routing_key(key) is True


# ====================================== per-chain anti-stuck (touch/pen split)

class TestPerChainAntiStuck:
    """The regression the touch/pen split exposed: anti-stuck detected on
    the motor-level max across chains, so one frozen chain hid behind a
    moving sibling — the fuse never started while anything else changed,
    and the stuck value floored the merge for the whole encounter.
    Detection and fade are per chain now."""

    PARAMS_BOTH = {
        "OGB/Orf/Boob/TouchOthersClose": True,
        "OGB/Orf/Boob/TouchOthers": 0.6,       # touch: will freeze here
        "OGB/Orf/Boob/PenOthers": 0.5,         # pen: kept moving by tests
    }
    ZONES = {("Orf", "Boob")}
    ...
    def _cfg(self):
        ...
        import copy as _c
        pen = _c.deepcopy(chain)
        pen["type"] = "penetration"
        touch = _c.deepcopy(chain)
        touch["type"] = "touch"
        cfg["mix"] = {"0": {"chains": [pen, touch], "merge": "max"}}
        return cfg

    ANTISTUCK = {"enabled": True, "active_s": 2.0, "peaked_s": 15.0}

    def _tick(self, router, cfg, params, clock, advance=0.25):
        out = router._calculate_motor_target(
            "dev", 0, cfg, params, zones=self.ZONES,
            antistuck=self.ANTISTUCK)
        clock.advance(advance)
        return out

    def test_frozen_touch_fades_while_pen_still_moves(self, router, clock):
        """The user-reported case: touch freezes mid-encounter, pen keeps
        stroking. The moving pen must NOT keep resetting the touch
        chain's fuse — after active_s the stuck 0.6 floor drops out and
        the motor follows the pen alone."""
        cfg = self._cfg()
        params = dict(self.PARAMS_BOTH)
        pen_wave = [0.5, 0.1, 0.45, 0.15, 0.4, 0.2, 0.35, 0.25, 0.3, 0.3]
        for i in range(10):                     # 2.5s of pen movement
            params["OGB/Orf/Boob/PenOthers"] = pen_wave[i]
            out = self._tick(router, cfg, params, clock)
        # Touch has now been frozen at 0.6 for > active_s. The pen sits
        # at 0.3 — without per-chain detection the output would still be
        # floored at the stuck 0.6.
        params["OGB/Orf/Boob/PenOthers"] = 0.31
        out = self._tick(router, cfg, params, clock)
        assert out == pytest.approx(0.31, abs=0.02), (
            f"stuck touch floor survived: {out}")

    def test_pre_split_shape_would_have_stalled_the_fuse(self, router, clock):
        """Sanity on the diagnosis itself: with detection running on the
        MERGED max (one untyped chain seeing both contributions, as
        before the split), the same scenario never fires the fuse while
        pen moves above the frozen value."""
        cfg = _basic_motor_cfg()
        cfg["mix"]["0"]["chains"][0]["smoothing"] = {
            "rise_ms": 0.0, "fall_ms": 0.0}
        params = dict(self.PARAMS_BOTH)
        for i in range(10):
            # Pen swings ABOVE the frozen touch, so the merged max keeps
            # changing and the single chain's fuse keeps resetting.
            params["OGB/Orf/Boob/PenOthers"] = 0.9 if i % 2 else 0.7
            out = self._tick(router, cfg, params, clock)
        params["OGB/Orf/Boob/PenOthers"] = 0.0
        out = self._tick(router, cfg, params, clock)
        # Combined chain: pen gone, frozen touch still holds the value —
        # its own fuse only starts now.
        assert out == pytest.approx(0.6, abs=0.02)

    def test_resumed_touch_input_returns_instantly(self, router, clock):
        cfg = self._cfg()
        params = dict(self.PARAMS_BOTH)
        params["OGB/Orf/Boob/PenOthers"] = 0.0
        for _ in range(10):                     # freeze past the fuse
            out = self._tick(router, cfg, params, clock)
        assert out == 0.0                       # touch cut, pen silent
        params["OGB/Orf/Boob/TouchOthers"] = 0.7    # partner moves again
        out = self._tick(router, cfg, params, clock)
        assert out == pytest.approx(0.7, abs=0.02)

    def test_saturated_touch_gets_the_longer_fuse(self, router, clock):
        cfg = self._cfg()
        params = dict(self.PARAMS_BOTH)
        params["OGB/Orf/Boob/TouchOthers"] = 1.0    # saturated hold
        params["OGB/Orf/Boob/PenOthers"] = 0.0
        for _ in range(10):                     # 2.5s > active_s
            out = self._tick(router, cfg, params, clock)
        # Mid-range would be cut by now; saturated rides the peaked fuse.
        assert out == pytest.approx(1.0, abs=0.02)

    def test_contact_stream_drops_the_frozen_chain_too(self, router, clock):
        """The param-out mirror must not keep telling the avatar there is
        contact from an input that froze."""
        cfg = self._cfg()
        params = dict(self.PARAMS_BOTH)
        params["OGB/Orf/Boob/PenOthers"] = 0.0
        for _ in range(10):
            self._tick(router, cfg, params, clock)
        assert router.last_contact_outputs[("dev", 0)] == 0.0

    def test_fold_bar_meter_sees_the_fade(self, router, clock):
        """The per-chain emit carries the faded value, so the folded
        bar's meter shows the chain dying rather than stuck-alive."""
        cfg = self._cfg()
        captured = []
        router.subscribe_intermediates("dev", 0, 1, captured.append)
        params = dict(self.PARAMS_BOTH)
        params["OGB/Orf/Boob/PenOthers"] = 0.0
        for _ in range(10):
            self._tick(router, cfg, params, clock)
        assert captured[-1]["out"] == 0.0


class TestAntiStuckAggressiveMidrange:
    """The detection model's rationale, pinned: a real tracked value
    between zero and one can never hold still (animation/IK jitter it
    past the epsilon constantly, and slow drift accumulates), so an
    exact mid-range hold is a frozen sender and the default fuse is
    aggressive. Only saturated ~1.0 gets patience — all-the-way-in-and-
    held genuinely sits constant."""

    DEFAULTS = {"enabled": True}     # router fallbacks supply the times
    ...
    def _hold(self, router, clock, value, seconds, step=0.25):
        out = 0.0
        for _ in range(int(seconds / step)):
            out = router._calculate_motor_target(
                "dev", 0, self._cfg(), {"P": value}, zones=set(),
                antistuck=self.DEFAULTS)
            clock.advance(step)
        return out

    def test_midrange_hold_cut_within_two_seconds_by_default(self, router, clock):
        assert self._hold(router, clock, 0.6, 2.0) == 0.0

    def test_midrange_survives_under_a_second(self, router, clock):
        assert self._hold(router, clock, 0.6, 0.75) == pytest.approx(0.6)

    def test_saturated_hold_survives_the_midrange_fuse(self, router, clock):
        # 1.0 held for 5s: far past active_s, well inside peaked_s=10.
        assert self._hold(router, clock, 1.0, 5.0) == pytest.approx(1.0)

    def test_jitter_above_epsilon_never_trips_it(self, router, clock):
        # Real tracking: fourth-decimal wiggle resets the fuse forever.
        out = 0.0
        for i in range(20):                      # 5s of jittering hold
            v = 0.6 + (0.002 if i % 2 else -0.002)
            out = router._calculate_motor_target(
                "dev", 0, self._cfg(), {"P": v}, zones=set(),
                antistuck=self.DEFAULTS)
            clock.advance(0.25)
        assert out == pytest.approx(0.6, abs=0.01)

    def test_shipped_defaults_are_one_and_ten(self):
        from settings.app import DEFAULT_APP_SETTINGS
        assert DEFAULT_APP_SETTINGS["toy_antistuck_active_s"] == 1
        assert DEFAULT_APP_SETTINGS["toy_antistuck_peaked_s"] == 10


# ====================================== sidechain duck (Touch under Penetration)

class TestTouchDuck:
    """Optional: a Touch chain fades out while the Penetration chain on
    the same motor has something inside, and fades back over release_ms
    once it leaves. Off by default (OSC Goes Brrr takes the plain max)."""

    ZONES = {("Orf", "Boob")}

    def _cfg(self, duck_on=True, release_ms=400.0, duck_on_pen=False):
        import copy as _c
        cfg = _basic_motor_cfg()
        chain = {
            "depth": {"gain": 1.0, "curve": "linear", "curve_param": 1.0},
        ...
        pen["type"] = "penetration"
        if duck_on_pen:
            pen["duck"] = {"enabled": True, "release_ms": release_ms}
        touch = _c.deepcopy(chain)
        touch["type"] = "touch"
        touch["duck"] = {"enabled": duck_on, "release_ms": release_ms}
        cfg["mix"] = {"0": {"chains": [pen, touch], "merge": "max"}}
        return cfg

    def _params(self, touch, pen):
        return {
            "OGB/Orf/Boob/TouchOthersClose": True,
            "OGB/Orf/Boob/TouchOthers": touch,
            "OGB/Orf/Boob/PenOthers": pen,
        }

    def _tick(self, router, cfg, touch, pen, clock, n=1, dt=0.25):
        out = 0.0
        for _ in range(n):
            out = router._calculate_motor_target(
                "dev", 0, cfg, self._params(touch, pen), zones=self.ZONES)
            clock.advance(dt)
        return out

    def test_off_by_default_touch_floors_the_merge(self, router, clock):
        cfg = self._cfg(duck_on=False)
        out = self._tick(router, cfg, touch=0.6, pen=0.1, clock=clock, n=3)
        assert out == pytest.approx(0.6)

    def test_on_touch_fades_while_pen_inside(self, router, clock):
        cfg = self._cfg(duck_on=True)
        out = self._tick(router, cfg, touch=0.6, pen=0.1, clock=clock, n=3)
        assert out == pytest.approx(0.1)          # pen alone

    def test_releases_over_release_ms(self, router, clock):
        cfg = self._cfg(duck_on=True, release_ms=1000.0)
        self._tick(router, cfg, touch=0.6, pen=0.1, clock=clock, n=3)
        # Pen leaves; the envelope releases linearly over 1s (dt = 0.25),
        # so a couple of ticks in the touch is partway back...
        self._tick(router, cfg, touch=0.6, pen=0.0, clock=clock, n=1)
        out = self._tick(router, cfg, touch=0.6, pen=0.0, clock=clock, n=1)
        assert 0.1 < out < 0.55
        # ...and fully back after the release time has elapsed.
        out = self._tick(router, cfg, touch=0.6, pen=0.0, clock=clock, n=4)
        assert out == pytest.approx(0.6)

    def test_penetration_chain_never_ducks(self, router, clock):
        cfg = self._cfg(duck_on=False, duck_on_pen=True)
        out = self._tick(router, cfg, touch=0.0, pen=0.5, clock=clock, n=3)
        assert out == pytest.approx(0.5)

    def test_contact_stream_and_emit_carry_the_duck(self, router, clock):
        cfg = self._cfg(duck_on=True)
        captured = []
        router.subscribe_intermediates("dev", 0, 1, captured.append)
        self._tick(router, cfg, touch=0.6, pen=0.1, clock=clock, n=3)
        assert captured[-1]["duck"] == pytest.approx(1.0)
        assert captured[-1]["out"] == pytest.approx(0.0)
        assert router.last_contact_outputs[("dev", 0)] == pytest.approx(0.1)

    def test_pen_chain_emit_reports_no_duck(self, router, clock):
        cfg = self._cfg(duck_on=True)
        captured = []
        router.subscribe_intermediates("dev", 0, 0, captured.append)
        self._tick(router, cfg, touch=0.6, pen=0.1, clock=clock, n=3)
        assert captured[-1]["duck"] == 0.0

    def test_simulator_never_ducks_the_chain_being_tuned(self, router, clock):
        """The sidechain is the pen chain's REAL input, pre-provider."""
        cfg = self._cfg(duck_on=True)
        router.set_chain_value_provider("dev", 0, 0, lambda: 0.8)
        out = self._tick(router, cfg, touch=0.9, pen=0.0, clock=clock, n=3)
        assert out == pytest.approx(0.9)          # touch still alive

    def test_turning_it_off_mid_duck_releases_instantly(self, router, clock):
        on = self._cfg(duck_on=True, release_ms=3000.0)
        self._tick(router, on, touch=0.6, pen=0.1, clock=clock, n=3)
        off = self._cfg(duck_on=False)
        out = self._tick(router, off, touch=0.6, pen=0.1, clock=clock, n=1)
        assert out == pytest.approx(0.6)
