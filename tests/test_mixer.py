...
class TestSamplePatternEdge:
    ...
    def test_known_waveforms_covered_by_constant(self):
        # Sanity: the public WAVEFORMS tuple matches what we test. The four
        # periodic shapes are covered below; `random` (a deterministic
        # wander, still a pure function of t) has its own tests in
        # test_simulated_input.py.
        assert set(WAVEFORMS) == {"sine", "square", "triangle", "sawtooth", "random"}
